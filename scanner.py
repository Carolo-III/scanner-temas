...
def generate_analysis(data, anthropic_key):
    ...
    # PUNTO 10 — alertas CMF de seguimiento: setups de sesiones anteriores con CMF deteriorado
    evaluaciones = data.get('evaluaciones', [])
    alertas_cmf = [e for e in evaluaciones if e.get('alerta_cmf')]
    if alertas_cmf:
        summary += '\nSEGUIMIENTO — ALERTAS CMF (setups de sesiones anteriores con flujo deteriorado):\n'
        summary += 'Estos valores tuvieron setup valido en los ultimos dias pero el CMF ha caido por debajo de 0.05.\n'
        summary += 'Si tienes posicion abierta, considera reducir el 50% hasta que el CMF recupere.\n'
        for a in alertas_cmf:
            summary += (f'- {a["ticker"]}: setup del {a["fecha_setup"]} ({a["dias"]}d) | '
                        f'Ret: {a["ret_pct"]}% | CMF actual: {a["cmf_actual"]} | '
                        f'Estado: {a["resultado"]}\n')
        summary += '\n'
    ...
    prompt = (
        ...
        'PUNTO 10 — SEGUIMIENTO CMF: si aparece el bloque "SEGUIMIENTO — ALERTAS CMF" en los datos, anyadelo como una seccion separada ENTRE la seccion 4 (EXTENDIDOS) y la CONCLUSION, titulada exactamente "SEGUIMIENTO DE POSICIONES ABIERTAS". Si hay mas de 10 alertas, agrupa: menciona primero los 3-5 casos mas relevantes (mayor retorno positivo para proteger, o mayor retorno negativo para cortar), luego una frase de resumen del tipo "otros N valores con CMF deteriorado — misma regla de reduccion al 50%"; NO listes mas de 10 tickers de forma individual. Para cada ticker mencionado: explica que el CMF ha caido por debajo de 0.05, lo que sugiere que la presion compradora se esta debilitando; indica que si hay posicion abierta la regla es valorar reducir el tamano a la mitad hasta que el CMF recupere por encima de 0.05; cita el retorno actual (ret_pct). Si no hay alertas CMF, no incluyas esta seccion. Seccion operativa, no analitica. '
...
def main():
    ...
    if evaluaciones:
        ...
        # PUNTO 10 — alertas de deterioro CMF post-entrada
        alertas_cmf = [e for e in evaluaciones if e.get('alerta_cmf')]
        if alertas_cmf:

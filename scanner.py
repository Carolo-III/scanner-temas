    ...
    for gn, tickers in grps.items():
        for tk in tickers:
            ...
            res.append({'ticker':tk,'group':gn,'rs_4w':r4,'rs_13w':r13,'vol_z':vz,
                ...
                'riesgo':riesgo_val,'riesgo_motivo':riesgo_motivo,'rvol':rvol})
    return res
...
def generate_analysis(data, anthropic_key):
    ...
    prompt = (
        ...
        'PUNTO 10 — SEGUIMIENTO CMF: si aparece el bloque "SEGUIMIENTO — ALERTAS CMF" en los datos, anyadelo como una seccion separada ENTRE la seccion 4 (EXTENDIDOS) y la CONCLUSION, titulada exactamente "SEGUIMIENTO DE POSICIONES ABIERTAS". Para cada ticker en la lista: explica que el flujo comprador medido por el CMF ha caido por debajo del umbral de 0.05 desde el momento del setup, lo que sugiere que la presion compradora que sustentaba la entrada se esta debilitando; indica que si el lector tiene posicion abierta, la regla del sistema es valorar reducir el tamano a la mitad hasta que el CMF recupere niveles por encima de 0.05; cita el retorno actual (ret_pct) para dar contexto de si la posicion esta en positivo o negativo. Si no hay alertas CMF, no incluyas esta seccion. Esta seccion es operativa, no analitica: no hagas valoraciones generales del mercado aqui, solo el seguimiento concreto de los tickers afectados. '

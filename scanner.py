...
def download_prices(tickers, period='1y'):
    ...
    NOTA (fix 27/06): ademas de reintentar cuando la respuesta viene completamente vacia
    (raw.empty), ahora tambien reintenta cuando la respuesta NO esta vacia pero la fila mas
    reciente viene degradada (casi todos los tickers del lote con NaN en la ultima sesion).
    Sintoma detectado en produccion: yfinance puede devolver un DataFrame "no vacio" (pasa el
    chequeo anterior sin problema) pero con la fila de la sesion mas reciente incompleta para
    casi todo el lote — eso disparaba resultados degenerados en calc_market_breadth (100% sobre
    MM50 y MM200 simultaneamente, avance+descenso=1) sin que download_prices detectara nada raro,
    porque "no vacio" no es lo mismo que "completo". Umbral: si menos del 50% de los tickers del
    lote tienen dato valido en la ultima fila, se trata igual que una respuesta vacia (reintento
    si quedan intentos, se acepta con aviso si no quedan).
    """
    if not tickers:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    LOTE = 50
    UMBRAL_VALIDEZ_ULTIMA_FILA = 0.50
    ...
    for i, lote in enumerate(lotes):
        ...
        for intento in range(3):
            try:
                ...
                # NUEVO (27/06) — validar que la ultima fila no venga degradada antes de aceptarla
                frac_validos = c.iloc[-1].notna().mean() if len(c) > 0 and c.shape[1] > 0 else 0
                if frac_validos < UMBRAL_VALIDEZ_ULTIMA_FILA:
                    if intento < 2:
                        import time
                        print(f'datos degradados ({frac_validos*100:.0f}% valido), reintentando...', end=' ')
                        time.sleep(5); continue
                    else:
                        print(f'AVISO: ultima fila degradada tras 3 intentos ({frac_validos*100:.0f}% valido), se acepta igualmente')
                all_c.append(c); all_v.append(v)
                all_h.append(h); all_l.append(l)
                if len(lotes) > 1 and frac_validos >= UMBRAL_VALIDEZ_ULTIMA_FILA: print('OK')
                break

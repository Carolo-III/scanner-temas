...
def generate_analysis(data, anthropic_key):
    ...
    valid=[v for v in values if v.get('entry_range',{}).get('entry_lo') and (v.get('entry_range',{}).get('rr') or 0)>=2.0 and (v.get('sct') or 0)>=40]
    ...
    prompt = (
        ...
        'Si el ratio R/B de un setup es mas del doble de la media del resto de setups del informe, añade una nota explicita: el objetivo puede ser el maximo historico pre-colapso y no una resistencia tecnica real; indica en ese caso que conviene fijar un primer objetivo parcial en la MM200 o en el maximo reciente. '
        ...
        'Al final de la seccion ENTRADAS incluye una tabla comparativa con columnas: TICKER | SETUP | R/B | SCT | RIESGO | RANKING. RIESGO es OBLIGATORIO calcularlo con esta formula exacta sin excepciones: Alto = SCT<50 O RSI>70; Medio = SCT entre 50-70 Y RSI entre 55-70; Bajo = SCT>70 Y RSI<70 Y CMF>0.1. NO uses criterios propios para RIESGO — aplica la formula mecanicamente. RANKING ordena por conviccion total: SCT (40%) + R/B normalizado (20%) + fuerza sectorial del grupo (20%) + CMF (20%); indica la formula brevemente bajo la tabla. '

...
def generate_analysis(data, anthropic_key):
    ...
    prompt = (
        ...
        'Si CMF<0 y ADX>25 advierte de divergencia bajista — el scanner ya ha penalizado el SCT pero Claude debe mencionarlo explicitamente. '
        ...
        'Al final de la seccion ENTRADAS incluye una tabla comparativa con columnas: TICKER | SETUP | R/B | SCT | RIESGO | RANKING. RIESGO = Alto si RSI>70 o CMF<0, Medio si RSI 55-70 y CMF>0, Bajo si RSI<55 y CMF>0.1. RANKING ordena por conviccion total. '
        'Aviso final obligatorio: "Este analisis no constituye asesoramiento financiero."\n\n' + summary
    )
    import httpx
    msg=client.messages.create(model='claude-opus-4-8',max_tokens=2500,messages=[{'role':'user','content':prompt}], timeout=httpx.Timeout(120.0, connect=30.0))

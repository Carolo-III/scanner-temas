    ...
    if macro:
        ...
        if u30.get('alerta'):
            ...
            macro_txt += (f'- ALERTA DE REGIMEN — US30Y (bono 30 años): {u30.get("nivel")}% | '
                          f'{u30.get("chg_20s_pb"):+} pb en 20 sesiones{extra}\n')
        wti_r = reg.get('wti', {})
        if wti_r.get('alerta'):
...
def main():
    ...
    if evaluaciones:
        ...
        if cmf_pre:
            print(f'  ℹ️  {len(cmf_pre)} setups con CMF ya negativo AL CREARSE (sin alerta blanda; '

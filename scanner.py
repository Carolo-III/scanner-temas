...
def calc_supertrend(high, low, close, period=10, multiplier=3):
    ...
    try:
        ...
        hl2 = (h + l) / 2
        banda_sup_basica = hl2 + multiplier * atr
        banda_inf_basica = hl2 - multiplier * atr

        primero_valido = atr.first_valid_index()
        if primero_valido is None: return None
        inicio = idx.get_loc(primero_valido)

        banda_sup_final = banda_sup_basica.copy()
        banda_inf_final = banda_inf_basica.copy()
        direccion = ['alcista'] * len(c)

        for i in range(inicio + 1, len(c)):
            if banda_sup_basica.iloc[i] < banda_sup_final.iloc[i-1] or c.iloc[i-1] > banda_sup_final.iloc[i-1]:
                banda_sup_final.iloc[i] = banda_sup_basica.iloc[i]
            else:
                banda_sup_final.iloc[i] = banda_sup_final.iloc[i-1]
            if banda_inf_basica.iloc[i] > banda_inf_final.iloc[i-1] or c.iloc[i-1] < banda_inf_final.iloc[i-1]:
                banda_inf_final.iloc[i] = banda_inf_basica.iloc[i]
            else:
                banda_inf_final.iloc[i] = banda_inf_final.iloc[i-1]

            if c.iloc[i] > banda_sup_final.iloc[i-1]:
                direccion[i] = 'alcista'
            elif c.iloc[i] < banda_inf_final.iloc[i-1]:
                direccion[i] = 'bajista'
            else:
                direccion[i] = direccion[i-1]

        dir_final = direccion[-1]
        nivel = float(banda_inf_final.iloc[-1]) if dir_final == 'alcista' else float(banda_sup_final.iloc[-1])

...
def calc_cmf(high, low, close, volume, period=20):
    try:
        h = high.dropna(); l = low.dropna()
        c = close.dropna(); v = volume.dropna()
...
def generate_analysis(data, anthropic_key):
    ...
    prompt = (
        ...
        'IMPORTANTE: al final de CADA seccion añade exactamente: '
        'una linea en blanco, luego "En palabras simples:" y una explicacion de 2 frases sin jerga. '
        ...
        'SQUEEZE=compresion de volatilidad, ruptura inminente potente. '
        'una linea en blanco, luego el emoji seguido de "En palabras simples:" '
        'y una explicacion de 2 frases sin jerga financiera, como si se lo contaras '
        'a alguien que nunca ha invertido. Usa analogias del dia a dia. '
        'Max 500 palabras en total. '
        'Aviso final obligatorio: "Este analisis no constituye asesoramiento financiero."\n\n' + summary
    )
    msg=client.messages.create(model='claude-sonnet-4-5',max_tokens=1500,messages=[{'role':'user','content':prompt}], timeout=60.0)
...
def main():
    ...
    upload_to_github('setups_history.json', json.dumps(setups_history, ensure_ascii=False))
    if evaluaciones:
    ...
    data={'timestamp':ts,'mode':'S&P500 + Watchlist','groups':all_groups,'values':sorted(ar,key=lambda x:x['score'] or 0,reverse=True),'analisis':analisis,'spy_healthy':spy_ok,'evaluaciones':evaluaciones,'spy_ret_1w':spy_ret_1w,'spy_ret_1m':spy_ret_1m,'macro':macro}
    print('\n▸ Subiendo a GitHub...')

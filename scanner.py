...
def merge_breadth_entry(bh, entrada, max_entradas=500):
    """Fusiona una entrada en la serie: las reejecuciones del mismo dia SUSTITUYEN la
    entrada de esa fecha (el ultimo run del dia manda — normalmente la nocturna, la del
    cierre), orden cronologico, y tope de tamaño para que el fichero no crezca sin fin."""
    bh = [e for e in (bh or []) if e.get('fecha') != entrada.get('fecha')]
    bh.append(entrada)
...
def generate_analysis(data, anthropic_key):
    ...
    prompt = (
        ...
        'PUNTO 19 — DESGLOSE DEL RANKING: cada setup incluye junto al RANKING su desglose entre parentesis (SCT x/40 + R/B x/20 + sector x/20 + CMF x/20; los cuatro componentes suman exactamente el ranking). Usalo cuando compares candidatos entre si o expliques por que uno puntua mas que otro: cita el componente REAL que marca la diferencia (ej. "X supera a Y casi integramente por el componente sectorial") en vez de atribuirla a causas genericas. No recites el desglose completo de cada setup en el texto — la tabla comparativa ya lleva el RANKING total; el desglose es para fundamentar comparaciones concretas. NUNCA inventes pesos ni componentes distintos de los cuatro dados. '

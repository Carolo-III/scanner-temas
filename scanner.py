...
def _texto_putcall():
    ...
    # A diferencia del nivel, el CAMBIO es interpretable desde la primera sesion de serie.
    if eq is not None and len(hist) >= 2:
        _prev = hist[-2]['equity'] if hist[-1].get('equity') == eq else hist[-1]['equity']
        _delta = round(eq - _prev, 3)
        if _delta > 0:
            _dir = 'SUBE, es decir el sentimiento se mueve hacia el MIEDO'
        elif _delta < 0:
            _dir = 'BAJA, es decir el sentimiento se mueve hacia la COMPLACENCIA'
        else:
            _dir = 'no cambia'

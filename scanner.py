...
def calc_riesgo(sct, rsi, cmf, adx):
    ...
    medio = (70 <= sct <= 79) or (68 < rsi <= 70) or (0.05 <= cmf <= 0.09) or (17 <= adx <= 19)
    if medio:
        return 'MEDIO'
    ...
    try:
        ...
        if bi.get('breakout') and bl and price>bl*1.15:
            ...
            trigger = (f'Vigilar reentrada cuando precio quede dentro de ±4% de MM50 (${round(ma50,2)}) '
                       ...
                       if ma50 else 'Esperar pullback a una referencia tecnica clara antes de reevaluar.'
...
def analyze_universe(grps, bench, close_df, vol_df, high_df=None, low_df=None, spy_healthy=True):
    ...
    for gn, tickers in grps.items():
        for tk in tickers:
            ...
            riesgo_val = calc_riesgo(sct_val, rsi_val, cmf_val, adx_val)
            res.append({'ticker':tk,'group':gn,'rs_4w':r4,'rs_13w':r13,'vol_z':vz,
                ...
                'cmf':cmf_val,'atr':atr_val,'squeeze':squeeze,'pct_b':pct_b,'riesgo':riesgo_val})
    return res
...
def generate_analysis(data, anthropic_key):
    ...
    for v in valid[:5]:
        ...
        summary+=f' | RIESGO:{v.get("riesgo","?")} | RANKING:{v.get("ranking","?")}'
    ...
    prompt = (
        ...
        'Al final de la seccion ENTRADAS incluye una tabla comparativa con columnas: TICKER | SETUP | R/B | SCT | RIESGO | RANKING. RIESGO y RANKING vienen YA CALCULADOS en los datos (campos RIESGO: y RANKING: de cada setup) — usalos exactamente como aparecen, NO los recalcules ni apliques ninguna formula propia sobre ellos; tu unica tarea es transcribirlos a la tabla y, si es relevante, comentar brevemente que factores los explican (ej. "RIESGO BAJO porque SCT>=80, RSI en zona optima, CMF y ADX confirman tendencia"). '

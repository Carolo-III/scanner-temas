...
def anotar_tendencia_en_breadth(breadth):
    ...
    serie_disp = [e.get('dispersion_ratio') for e in bh] + [disp_hoy]
    serie_mcc = [e.get('mcclellan') for e in bh] + [breadth.get('mcclellan')]
    ...
    if breadth and breadth.get('pct_sobre_mm50') is not None:
        ...
        if _disp.get('ratio') is not None:
            breadth_txt += (
                ...
                f'La DIRECCION importa tanto como el nivel: si el ratio SUBE, el mercado se esta '
                f'volviendo MENOS disperso (los valores convergen con el indice) y la ventaja de '
                f'seleccionar valores se estrecha; si BAJA, la dispersion aumenta y esa ventaja crece. '
                f'No describas el nivel como estable si el contador temporal dice que lleva sesiones '
                f'moviendose. '
        ...
        if _tend:
            breadth_txt += (
                f'- CONTEXTO TEMPORAL: la dispersion lleva {_tend["sesiones_dispersion"]} sesiones '
                f'en tendencia {_tend["dispersion"]}; el McClellan, {_tend["sesiones_mcclellan"]} '
                f'sesiones en tendencia {_tend["mcclellan"]}. USA ESTE DATO: di si la amplitud es una '
                f'situacion NUEVA o SOSTENIDA, en vez de describir la cifra del dia como si fuera '
                f'informacion nueva. Si las dos direcciones NO coinciden, senala la divergencia y '
                f'cuantas sesiones lleva. Es descripcion del pasado, NO prediccion.\n')
        breadth_txt += '\n'
...
def main():
    ...
    breadth = anotar_tendencia_en_breadth(breadth)   # P37b — contexto temporal para el informe

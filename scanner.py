...
def _calc_macro_regimen():
    ...
    """
    tickers = ['^TYX', '^TNX', '^FVX', '^IRX', 'CL=F', 'HYG', 'IEF']
...
def merge_breadth_entry(bh, entrada, max_entradas=500):
    ...
    bh.append(entrada)
    return sorted(bh, key=lambda e: e.get('fecha') or '')[-max_entradas:]
...
def validar_informe(analisis, finalistas):
    ...
    entre los finalistas sin que nadie lo detecte. Esta función comprueba:
      - Presencia de las secciones obligatorias del prompt
      - Que los tickers del informe estén en el conjunto de finalistas
    ...
    tickers_finalistas = {v['ticker'] for v in finalistas} if finalistas else set()
    if tickers_finalistas:
        inventados = tickers_informe - tickers_finalistas
        # Permitir hasta 2 tickers fuera (contexto, comparaciones): solo avisa si hay más
        if len(inventados) > 2:
            avisos.append(f'Tickers en el informe no presentes en finalistas: {sorted(inventados)[:5]}')
    valido = len(avisos) == 0
...
def generate_analysis(data, anthropic_key):
    ...
    for v in valid[:5]:
        ...
        try:
            precio_obj = er.get('objetivo')
            precio_ent = er.get('entrada')
            atr_val = v.get('atr')
            if precio_obj and precio_ent and atr_val and atr_val > 0:
                distancia = abs(precio_obj - precio_ent)
                dias_base = round(distancia / atr_val, 1)
                # Rango: ±40% del ATR base (aproxima variabilidad del ATR en el último mes)
                dias_min = max(1, round(dias_base * 0.6))
                dias_max = round(dias_base * 1.4)
                if dias_max >= 1:
                    summary += f' | Tiempo estimado al objetivo: {dias_min}-{dias_max} días hábiles (ATR: ${round(atr_val,2)}/día)'
        except Exception:
            pass
    ...
    prompt = (
        ...
        'PUNTO 33 — TIEMPO ESTIMADO AL OBJETIVO: cada setup con R/B puede incluir la línea "Tiempo estimado al objetivo: X-Y días hábiles (ATR: $Z/día)". Es una referencia de escala temporal basada en la velocidad histórica del valor — NO una predicción. Úsala para contextualizar el R/B: un R/B de 1:4 a 5 días es muy diferente de uno a 60 días. Cítala una sola vez por setup, en el párrafo de niveles operativos, con la advertencia implícita de que es orientativa. Si el dato no aparece, no lo menciones. '
...
def main():
    ...
    _valido, _avisos = validar_informe(analisis, valid_final if 'valid_final' in vars() else [])
    if _avisos:
        for av in _avisos:

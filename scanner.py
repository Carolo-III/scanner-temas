...
def get_fundamentales_fmp(tickers, api_key=None):
    ...
    unicos campos sin equivalente en yfinance:
      ...
      - Serie de EPS de 8 trimestres   <- /stable/earnings
    ...
    for tk in tickers:
        ...
        earnings, motivo = _fmp_get('earnings', {'symbol': tk, 'limit': 12})
        if motivo == 'simbolo':
        ...
        if isinstance(earnings, list):
            ...
            serie = [e.get('epsActual') for e in reportados[:8]][::-1]  # antiguo -> reciente
            serie = [round(float(x), 2) for x in serie if x is not None]
            if len(serie) >= 4:
                f['fmp_eps_8q'] = serie
        result[tk] = f
    n_con_datos = sum(1 for v in result.values() if v)
    print(f'  FMP (complemento ROIC/EPS8T): datos para {n_con_datos}/{len(tickers)} tickers'
...
def generate_analysis(data, anthropic_key):
    ...
    # NUEVO (05/07) — filtro de volumen minimo absoluto: 500.000 acciones/dia de media 30d.
    # RVOL solo mide volumen relativo; sin este filtro un valor de base muy baja podia
    # llegar a ENTRADAS con RVOL alto pese a ser de los menos activos del universo.
    # Si no hay dato (historico <30 sesiones), no se penaliza — mismo criterio que _rvol_ok.
    UMBRAL_VOL_MINIMO = 500_000
    def _vol_minimo_ok(v):
        vm = v.get('vol_media_30d')
        return vm is None or vm >= UMBRAL_VOL_MINIMO
    ...
    # compartido con main()/Celda 3-4 (mutacion deliberada del mismo objeto), de modo que
    ...
    for v in valid[:5]:
        ...
        if fund:
            ...
            # calidad fundamental adicional (margenes via yfinance; ROIC/EPS8T via FMP opcional)
            ...
            if fund.get('fmp_eps_8q'):
                f_parts.append(f'EPS8T:{"->".join(str(x) for x in fund["fmp_eps_8q"])}')
    ...
    # FIX (11/07 tarde) — cruce con supertrend_pre_roto: un ticker puede aparecer aqui como
    # candidato "limpio" de EXTENDIDOS (basado solo en precio/MM50/RSI/CMF de hoy) mientras
    # ya tiene un setup abierto en setups_history.json marcado supertrend_pre_roto=true (nacio
    # con el Supertrend ya bajista). Caso real 11/07: $TMUS repitiendo el mismo pullback MM50
    # cinco sesiones seguidas (07-06 a 07-10), todas con supertrend_al_crear=bajista, mientras
    # el informe lo presentaba en EXTENDIDOS como oportunidad limpia sin ninguna referencia a
    # ese historial. calc_entry_range() no tiene visibilidad de setups_history.json (son dos
    # pipelines independientes: uno calcula el candidato de hoy, otro evalua setups pasados),
    # asi que el cruce se hace aqui, con los datos que ya estan cargados en esta funcion.
    evaluaciones = data.get('evaluaciones', [])
    pre_roto_por_ticker = {a['ticker']: a for a in dedup_alertas_por_ticker(
        [e for e in evaluaciones if e.get('supertrend_pre_roto')])}
    ...
    if extendidos:
        ...
        if vigilancia:
            ...
            for v in vigilancia[:5]:
                ...
                # FIX (11/07 tarde) — ver comentario junto a pre_roto_por_ticker mas arriba
                pr = pre_roto_por_ticker.get(v['ticker'])
                if pr:
                    dias_st = pr.get('supertrend_dias')
                    dias_txt = f'{dias_st} sesiones' if dias_st is not None else 'varias sesiones'
                    summary += (f' | AVISO SUPERTREND: ya tiene un setup abierto del {pr["fecha_setup"]} '
                                f'nacido con Supertrend bajista (lleva {dias_txt} asi, sin alerta porque la '
                                f'condicion ya existia al crearse) — el trigger de reentrada de arriba aun no '
                                f'lo ha confirmado, tratar con cautela adicional')
                summary+='\n'
        if severos:
    ...
    prompt = (
        ...
        'Si un candidato de VIGILANCIA ACTIVA incluye "AVISO SUPERTREND" en sus datos, es obligatorio recoger esa advertencia en su parrafo, con el mismo peso que el resto del trigger: indica que el ticker ya tiene un setup abierto con el Supertrend bajista desde origen (nacio roto, por eso no genera alerta de seguimiento) y que el trigger de reentrada por RSI/CMF todavia no lo ha confirmado. No lo redactes como nota al pie ni lo omitas por brevedad — es la diferencia entre presentar el ticker como candidato limpio o como candidato con una señal tecnica adversa ya activa en paralelo. '
...
def main():
    ...
    if evaluaciones:
        ...
        # FIX (11/07) — un mismo ticker puede tener VARIOS setups abiertos en fechas distintas
        # dentro de los 30 dias de historico (visto en produccion: PHM con 10 entradas entre
        # 06-09 y 07-06). alerta_supertrend y supertrend_pre_roto son mutuamente excluyentes
        # POR EVALUACION (ticker+fecha_setup), no por ticker — asi que el mismo ticker podia
        # aparecer a la vez en ALERTAS SUPERTREND (por un setup antiguo) y en la lista de
        # pre-rotos (por un setup mas reciente), sin fecha visible en esta segunda lista.
        # Leido en aislado (p.ej. desde el informe ejecutivo, que trabaja a nivel de ticker,
        # no de setup) esto se lee como una contradiccion aunque no lo sea. Fix: mismo
        # deduplicado por ticker (nos quedamos con el setup mas reciente) y misma fecha
        # visible en ambos bloques, para que quede trazable que son posiciones distintas.
        pre_rotos = dedup_alertas_por_ticker([e for e in evaluaciones if e.get('supertrend_pre_roto')])
        if alertas_st:
        ...
        if pre_rotos:
            print(f'  ℹ️  {len(pre_rotos)} tickers con Supertrend ya bajista AL CREARSE (sin alerta; '
                  f'registrados en data.json con supertrend_pre_roto=true): '
                  + ', '.join(f'{a["ticker"]} (setup {a["fecha_setup"]})' for a in pre_rotos))
    ...
    data={'timestamp':ts,'mode':'S&P500 + Watchlist','groups':all_groups,'values':sorted(ar,key=lambda x:x['score'] or 0,reverse=True),'analisis':analisis,'spy_healthy':spy_ok,'evaluaciones':evaluaciones,'spy_ret_1w':spy_ret_1w,'spy_ret_1m':spy_ret_1m,'macro':macro,'breadth':breadth,'fundamentales':fundamentales if "fundamentales" in vars() else {}}
    print('\n▸ Subiendo a GitHub...')

...
def update_setups_history(values, all_groups):
    ...
    for entrada in history[:-1]:  # todos menos hoy
        ...
        for s in entrada['setups']:
            ...
            supertrend_pre_roto = es_bajista_abierto and not ruptura_posterior
            # PUNTO 10 (RECALIBRADO 06/07) — alerta blanda: DISTRIBUCION CONFIRMADA, no cruce
            # de un dia. Antes: CMF<0.05 en la ultima sesion -> disparaba en el 67% de las
            # posiciones abiertas (100/150 el 05/07), incluidas ganadoras de +16% con CMF -0.003
            # y valores a milesimas del umbral: no discriminaba nada. Ahora: CMF<0 durante
            # CMF_DIAS_ALERTA sesiones consecutivas. Ademas, jerarquia de señales: si hay señal
            # dura activa (salida total), la blanda (reducir 50%) se suprime — nunca emitir
            # instrucciones contradictorias sobre el mismo ticker.
            CMF_DIAS_ALERTA = 3
...
def main():
    ...
    if evaluaciones:
        ...
        # PUNTO 10 + NUEVO (27/06, RECALIBRADO 06/07) — alertas de seguimiento con jerarquia:
        # dura (Supertrend, solo rupturas posteriores al setup) absorbe a blanda (CMF<0 3+ sesiones)
        ...
        if pre_rotos:
            print(f'  ℹ️  {len(pre_rotos)} setups con Supertrend ya bajista AL CREARSE (sin alerta; '

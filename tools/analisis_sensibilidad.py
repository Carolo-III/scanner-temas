...
# Umbral vigente en produccion. Se usa como referencia para el conteo de
# cambios de lado. Si cambia en scanner.py, cambiar aqui tambien.
UMBRAL_ACTUAL = 0.30
...
CLAVES_FECHA = ("fecha_creacion", "fecha", "fecha_alta", "creado")
...
def cargar_setups(ruta):
    """Carga setups_history.json y devuelve una lista plana de registros.

    Tolera dos formas: lista de registros, o dict con los registros
    colgando de alguna clave de primer nivel.
    """
    ...
    if isinstance(datos, dict):
        # Caso 1: dict de listas -> aplanar todas las listas de dicts.
        acumulado = []
        for valor in datos.values():
            if isinstance(valor, list):
                acumulado.extend(x for x in valor if isinstance(x, dict))

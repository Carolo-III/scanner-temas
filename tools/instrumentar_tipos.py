...
Uso:
    python tools/instrumentar_tipos.py                 # actualiza
    python tools/instrumentar_tipos.py --sembrar 120   # siembra historico
...
def descargar(dias):
    """Descarga los tramos via yfinance. Devuelve (fecha, niveles, series).

    NO PROBADO SIN RED. Si yfinance no esta disponible o algun tramo falla,
    se omite ese tramo y se sigue con los demas.
    """
    try:
    ...
    for clave, ticker in sorted(TRAMOS.items()):
        try:
            ...
            if hist is None or hist.empty:
                ...
                continue
            cierres = [float(x) for x in hist["Close"].tolist()
                       if x == x]  # descarta NaN
            if not cierres:
                print("  AVISO: %s sin cierres validos, se omite" % clave)
                continue
            niveles[clave] = cierres[-1]
            series[clave] = cierres
            fechas.append(hist.index[-1].strftime("%Y-%m-%d"))
            print("  OK %-6s %s  %.3f%%  (%d sesiones)"
                  % (clave, ticker, cierres[-1], len(cierres)))
        except Exception as exc:  # noqa: BLE001 - queremos degradar, no abortar
            print("  AVISO: fallo en %s (%s): %s" % (clave, ticker, exc))

    if not niveles:
        raise RuntimeError("Ningun tramo devolvio datos.")

    # La fecha de la entrada es la sesion del dato, no el reloj.
    fecha = max(fechas) if fechas else datetime.now().strftime("%Y-%m-%d")
    return fecha, niveles, series
...
def main(argv=None):
    ...
    if args.simular:
        ...
        fecha = sim["fecha"]
        niveles = sim["niveles"]
        series = sim.get("series", {})
    else:
        print("Descargando tramos de tipos...")
        try:
            fecha, niveles, series = descargar(args.sembrar)
        except RuntimeError as exc:
            print("ERROR: %s" % exc, file=sys.stderr)
            return 1

    entrada = construir_entrada(fecha, niveles, series, es_cierre=args.cierre)
    historico, accion = fusionar(historico, entrada)

    if accion == "ignorada_por_cierre":
        print("Entrada de %s IGNORADA: ya hay cierre para esa sesion." % fecha)
        return 0

    guardar_historico(args.fichero, historico)
    print("Entrada de %s %s. Total: %d entradas."
          % (fecha, accion, len(historico)))
    print()
    print(resumir(historico))
    return 0

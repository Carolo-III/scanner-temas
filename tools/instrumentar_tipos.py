...
===========================================================================
NO ES CAMINO DE PRODUCCION.  Retirado el 01/08/2026.
---------------------------------------------------------------------------
La serie diaria de tipos la genera AHORA el propio scanner:

    scanner.py / Celda 2  ->  _calc_macro_regimen()  produce reg['curva']
                              construir_entrada_rates()
                              merge_rates_entry()
                              actualizar_rates_history()
    main() / Celda 4      ->  rates_history.json en el commit unico

Ese es el unico camino que debe escribir rates_history.json a diario. El
scanner reutiliza la descarga que ya hacia para ^TYX (sin peticiones extra)
y hereda la guarda de normalizacion de decimas de Yahoo, que este script
NO tiene.

Este fichero se conserva SOLO para resembrar historico a demanda: bajar N
meses de golpe y regenerar la serie desde cero (se hizo el 01/08/2026 con
120 sesiones, 09/02 a 31/07). Fuera de ese uso, no ejecutarlo: escribe el
mismo fichero por una ruta paralela y sin la guarda de decimas.

Si algun dia hay que tocar la logica de tipos, se toca en scanner.py y en
la Celda 2 (paridad AST). Este script quedara desactualizado; asumirlo.
===========================================================================
...
def main(argv=None):
    ...
    if not args.resumen:
        print("AVISO: este script NO es el camino de produccion. La serie "
              "diaria la\n       escribe scanner.py. Uso legitimo aqui: "
              "resembrar historico.\n")
